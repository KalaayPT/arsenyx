  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if ((e.ctrlKey || e.metaKey) && !e.shiftKey && !e.altKey) {
      ...
      if (key === "b" || key === "i") {
        e.preventDefault();
        const wrap = key === "b" ? "**" : "_";
        const selected = v.slice(start, end) || (key === "b" ? "bold" : "italic");
        const next = v.slice(0, start) + wrap + selected + wrap + v.slice(end);
        applyChange(next, start + wrap.length, start + wrap.length + selected.length);
        return;
      }
    }
...
function MarkdownPreview({ source }: { source: string }) {
  return (
    <div className="prose prose-sm dark:prose-invert max-w-none [&_a]:text-primary [&_a]:underline [&_code]:rounded [&_code]:bg-muted [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-xs [&_h1]:mt-0 [&_h1]:mb-2 [&_h1]:text-xl [&_h1]:font-bold [&_h2]:mt-4 [&_h2]:mb-2 [&_h2]:text-lg [&_h2]:font-semibold [&_h3]:mt-3 [&_h3]:mb-1 [&_h3]:font-semibold [&_li]:ml-4 [&_li]:list-disc [&_ol_li]:list-decimal [&_p]:mb-2 [&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre]:rounded [&_pre]:bg-muted [&_pre]:p-2 [&_pre]:text-xs [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_ul]:mb-2 [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:mb-2 [&_ol]:list-decimal [&_ol]:pl-4 [&_blockquote]:border-l-2 [&_blockquote]:border-border [&_blockquote]:pl-3 [&_blockquote]:text-muted-foreground [&_table]:w-full [&_th]:border [&_th]:border-border [&_th]:px-2 [&_th]:py-1 [&_td]:border [&_td]:border-border [&_td]:px-2 [&_td]:py-1">
      <ReactMarkdown
