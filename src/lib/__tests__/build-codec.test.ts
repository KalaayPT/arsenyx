...
describe("encodeBuild", () => {
  ...
  it("encodes item information", () => {
    ...
    expect(decoded?.itemUniqueName).toBe("/Lotus/Powersuits/Excalibur/Excalibur");
  ...
  it("encodes aura slot", () => {
    ...
    expect(decoded?.auraSlot?.mod?.uniqueName).toBe("/Lotus/Upgrades/Mods/Aura/SteelCharge");
  });
  ...
  it("encodes arcanes", () => {
    const build = createEmptyBuildState();
    build.arcaneSlots = [
      { uniqueName: "/Lotus/Upgrades/Arcanes/ArcaneEnergize", name: "Arcane Energize", rank: 5, rarity: "Legendary" },
      { uniqueName: "/Lotus/Upgrades/Arcanes/ArcaneGrace", name: "Arcane Grace", rank: 3, rarity: "Legendary" },
    ];
    ...
    expect(decoded?.arcaneSlots?.[0].uniqueName).toBe("/Lotus/Upgrades/Arcanes/ArcaneEnergize");
  ...
  it("produces base64 output that can be URL-encoded", () => {
    const build = createEmptyBuildState({ buildName: "Test+Build/With?Special&Chars" });
    const encoded = encodeBuild(build);
...
describe("decodeBuild", () => {
  ...
  it("returns null for unknown version", () => {
    const futureVersion = { v: 99, i: "/Lotus/Test", c: "warframes", r: false, s: [] };
    const encoded = Buffer.from(JSON.stringify(futureVersion)).toString("base64");
...
describe("edge cases", () => {
  ...
  it("handles all 8 mod slots filled", () => {
    ...
    for (let i = 0; i < 8; i++) {
      expect(decoded?.normalSlots?.[i].mod?.uniqueName).toBe(`/Lotus/Test/Mod${i}`);
